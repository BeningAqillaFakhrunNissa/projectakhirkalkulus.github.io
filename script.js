...
function login() {
  ...
  // Cek apakah hari ini sama dengan hari terakhir login
  if (found.lastDate === today) {
    // Gunakan data hari ini yang tersimpan
    pemasukanHariIni = found.pemasukanHariIni || 0;
    pengeluaranHariIni = found.pengeluaranHariIni || 0;
  } else {
    // Hari berbeda, reset data harian
    pemasukanHariIni = 0;
    pengeluaranHariIni = 0;
  }
  ...
  tabelRiwayat.innerHTML = `
    <tr>
      <th>Waktu</th><th>Jenis</th><th>Jumlah</th>
    </tr>`;
  ...
  restoreChartFromRiwayat(found.transaksi);
...
function formatInputRupiah(input) {
  // Ambil nilai dan hapus karakter non-angka
...
/* Masukkan kembali data grafik dari transaksi lama */
function restoreChartFromRiwayat(transaksi) {
  const map = {};
  ...
  transaksi.forEach(item => {
    ...
    if (!map[tanggal])
      map[tanggal] = { pemasukan: 0, pengeluaran: 0 };

    if (item.jenis === "Pemasukan")
      map[tanggal].pemasukan += Math.abs(item.jumlah);
    else
      map[tanggal].pengeluaran += Math.abs(item.jumlah);
  });

  chartKeuangan.data.labels = Object.keys(map);
  chartKeuangan.data.datasets[0].data = Object.values(map).map(m => m.pemasukan);
  chartKeuangan.data.datasets[1].data = Object.values(map).map(m => m.pengeluaran);
...
function tambahRiwayat(jenis, jumlah, cssClass) {
  const row = tabelRiwayat.insertRow(1);
...
function loadRiwayat(list) {
  ...
  list.forEach(item => {
    const kelas = item.jumlah > 0 ? "plus" : "minus";
    tambahRiwayat(item.jenis, item.jumlah, kelas);
  });
}
...
function saveData() {
  ...
  users[idx].lastDate = today;
  users[idx].pemasukanHariIni = pemasukanHariIni;
  users[idx].pengeluaranHariIni = pengeluaranHariIni;

  users[idx].transaksi = [];

  for (let i = 1; i < tabelRiwayat.rows.length; i++) {
    const jenis = tabelRiwayat.rows[i].cells[1].textContent;
    let jumlah = parseRupiah(tabelRiwayat.rows[i].cells[2].textContent);

    if (jenis === "Pengeluaran") jumlah = -Math.abs(jumlah);

    users[idx].transaksi.push({ jenis, jumlah });
  }
